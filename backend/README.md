...
- **Method:** `GET` or `POST`
- **Content-Type:** `application/json` (for POST) or Query Parameters (for GET)
...
#### Sample GET Query Request

```bash
curl -X GET "https://project-mn.onrender.com/api/query?user_id=8a7b9c1d-2e3f-4a5b-6c7d-8e9f0a1b2c3d&query=Explain+the+technology+stack"
```
...
#### Sample GET Quiz Request

```bash
curl -X GET "https://project-mn.onrender.com/api/quiz?user_id=8a7b9c1d-2e3f-4a5b-6c7d-8e9f0a1b2c3d"
```
...
#### Sample POST Flashcards Request
...
#### Sample GET Flashcards Request

```bash
curl -X GET "https://project-mn.onrender.com/api/flashcards?user_id=8a7b9c1d-2e3f-4a5b-6c7d-8e9f0a1b2c3d"
```
...
const askQuestion = async (userQuery, userId) => {
  try {
    // Easily call via GET request query parameters:
    const response = await fetch(`${API_BASE_URL}/query?user_id=${userId}&query=${encodeURIComponent(userQuery)}`);
